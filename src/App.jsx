  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <Card>
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <div>
            <div style={{fontSize:11,color:C.textMuted,marginBottom:6,letterSpacing:1,textTransform:'uppercase'}}>Nombre de usuario</div>
            <div style={{background:C.surfaceHigh,borderRadius:10,padding:'11px 14px',color:C.textDim,fontSize:15}}>{session.name}</div>
            <div style={{fontSize:11,color:C.textDim,marginTop:4}}>El nombre de usuario no se puede cambiar</div>
          </div>
          <div>
            <div style={{fontSize:11,color:C.textMuted,marginBottom:6,letterSpacing:1,textTransform:'uppercase'}}>Nombre para el ranking</div>
            <Input value={displayName} onChange={e=>setDisplayName(e.target.value)} placeholder={`Por defecto: ${session.name}`}/>
            <div style={{fontSize:11,color:C.textDim,marginTop:4}}>Este es el nombre que verán todos en la clasificación</div>
          </div>
          <div>
            <div style={{fontSize:11,color:C.textMuted,marginBottom:6,letterSpacing:1,textTransform:'uppercase'}}>Nueva contraseña (opcional)</div>
            <Input type="password" value={newPass} onChange={e=>setNewPass(e.target.value)} placeholder="Dejar vacío para no cambiar"/>
          </div>
      ...
      <Card style={{borderColor:`${C.accent}44`}}>
        ...
        {!confirmDelete?(
          <Btn onClick={()=>setConfirmDelete(true)} variant='danger' full>🗑 Eliminar mi cuenta</Btn>
        ):(
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            ...
            <div style={{display:'flex',gap:8}}>
              <Btn onClick={()=>setConfirmDelete(false)} variant='ghost' full>Cancelar</Btn>
              <Btn onClick={deleteAccount} variant='danger' full disabled={loading}>Sí, eliminar</Btn>
            </div>
          </div>
        )}
      </Card>
...
function Leaderboard({users,betsMap,results,currentUserId}){
  const scored=users.filter(u=>u.role!=='admin').map(u=>{
    const{total,breakdown}=calcScore(betsMap[u.id]||{},results)
    return{...u,total,breakdown}
          ...
          <div key={u.id} style={{background:u.id===currentUserId?`${C.accent}18`:C.surface,border:`1px solid ${u.id===currentUserId?C.accent:C.border}`,borderRadius:14,padding:'14px 16px',boxShadow:i===0&&u.total>0?`0 0 24px ${C.gold}22`:'none'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div style={{display:'flex',alignItems:'center',gap:12}}>
                ...
                <div>
                  <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:18,color:u.id===currentUserId?C.accentHover:C.text}}>
                    {display}
                    {u.id===currentUserId&&<span style={{fontSize:11,color:C.textMuted}}> (tú)</span>}
                  </div>
...
function GroupBets({groupBets,onChange,disabled}){
  ...
  function handleChange(g,nb){
    setLocal(prev=>({...prev,[g]:nb}))
    onChange(g,nb)
  }
  return(
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <Card style={{borderColor:`${C.blue}44`}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:15,color:'#5bb3f5',marginBottom:8}}>🎲 Simulador</div>
        <div style={{fontSize:12,color:C.textMuted,marginBottom:12}}>Genera resultados aleatorios para toda la competición. Útil para probar la app antes del torneo.</div>
        <Btn onClick={simulate} disabled={simulating} variant='gold' full>{simulating?'Simulando…':'🎲 Simular competición completa'}</Btn>
      </Card>
      ...
      {activePhase!=='groups'&&<Card><div style={{color:C.textMuted,textAlign:'center',padding:20,fontSize:13}}>Los resultados de eliminatorias se generan automáticamente al simular o al introducir los clasificados de grupos.</div></Card>}
  ...
  if(showProfile)return(
    <div style={{minHeight:'100vh',background:C.bg,paddingBottom:72}}>
      ...
      <main style={{maxWidth:560,margin:'0 auto',padding:'16px 14px'}}>
        <ProfileScreen
          session={session}
          onUpdate={updated=>{setSession(updated);loadAll()}}
          onDelete={()=>{localStorage.removeItem('porra_user');setSession(null)}}
